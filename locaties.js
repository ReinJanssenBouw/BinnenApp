  ...
  window.BinnenLocaties = { mount(el, api) {
    ...
    function render() {
      ...
      el.innerHTML = `<section class="loc-panel" data-loc-root>
        <div class="loc-heading"><div><h1>Locatie</h1><p>Stel per stelling de rijen (Y) en productkolommen (X) in.</p></div></div>
        ...
        <div class="loc-racks">${state.racks.map((r,i)=>`<section class="loc-rack" data-rack="${i}">
          ...
          <label>Productkolommen (X)<input data-field="columns" value="${esc(r.columns)}" type="number" inputmode="numeric" min="1" max="50" step="1" ${!admin()||state.busy?'disabled':''}></label></div>
    ...
    function validRack(r) {
      return r.name.trim().length>0 && r.name.trim().length<=64 && Number.isInteger(Number(r.rows)) && Number(r.rows)>=1 && Number(r.rows)<=50 && Number.isInteger(Number(r.columns)) && Number(r.columns)>=1 && Number(r.columns)<=50;
    }
    function preview(r) {
      if (!validRack(r)) return '<p>Vul een naam en 1 tot 50 rijen en kolommen in.</p>';
      let cells='';
      for(let y=Number(r.rows);y>=1;y--)for(let x=1;x<=Number(r.columns);x++)cells+=`<div class="loc-cell">X${x} · Y${y}</div>`;
      return `<p class="loc-axis">Y: van onder naar boven · X: van links naar rechts</p><div class="loc-grid-scroll"><div class="loc-grid" style="grid-template-columns:repeat(${Number(r.columns)},minmax(76px,1fr))">${cells}</div></div>`;
    }
    async function load() {
    ...
    el.addEventListener('input',e=>{
      ...
      if(!['name','rows','columns'].includes(field)||!row||!admin()||state.busy)return;
      state.racks[Number(row.dataset.rack)][field]=e.target.value;
    ...
    el.addEventListener('click',async e=>{
      ...
      if(action==='add'){
        ...
        state.racks.push({id:crypto.randomUUID(),name:`Stelling ${n}`,rows:1,columns:1});state.dirty=true;state.message='Onopgeslagen wijzigingen';render();
        ...
        try{const d=await api.save(state.racks.map(r=>({...r,name:r.name.trim(),rows:Number(r.rows),columns:Number(r.columns)})),state.revision);state.racks=d.racks;state.revision=d.revision;state.dirty=false;state.message='Indeling opgeslagen.';}
