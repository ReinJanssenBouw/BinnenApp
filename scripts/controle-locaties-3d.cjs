...
app.whenReady().then(async()=>{
 ...
 return {defaultHeight,wallHeightPersisted,wallHeightValidated,rotationWarned,roomLabel,outsideWarned,insideCleared,invalidBlocked,assigned,warned,persisted,moved,removed,conflict,flat,webgl,overflow,saved,saves,assigns,stock:db.products.map(p=>[p.stock,p.min_stock]),status:q('.l3-status').textContent};
 })()`);
 console.log(JSON.stringify(result));
 for(const k of ['defaultHeight','wallHeightPersisted','wallHeightValidated','rotationWarned','roomLabel','outsideWarned','insideCleared','invalidBlocked','assigned','warned','persisted','moved','removed','conflict','flat','webgl'])assert(result[k],k);
