...
(() => {
  ...
  window.BinnenLocaties3D={mount(el,api){
    ...
    const state={data:null,rackId:null,x:null,y:null,productId:null,dirty:false,busy:false,mode:'3d',query:'',overview:false,message:'Stellingen laden…'};
    let view=null,closed=false,viewError='';
    el.innerHTML=`<section class="l3-root"><header class="l3-header"><div><span class="l3-eyebrow">LOCATIE · MAGAZIJN</span><h1>Je magazijn in 3D</h1></div><div class="l3-head-actions"><button type="button" data-l3="mode">Vakkenlijst</button><button type="button" data-l3="reload">Vernieuwen</button><button type="button" data-l3="save" class="l3-primary" disabled>Model opslaan</button></div></header><p class="l3-status" role="status" aria-live="polite"></p><p class="l3-storage" hidden></p><div class="l3-work"><aside class="l3-racks" aria-label="Stellingen"></aside><div class="l3-stage"><div class="l3-views" aria-label="Camerastand"><button type="button" data-l3="view" data-view="perspective">3D</button><button type="button" data-l3="view" data-view="front">Voorkant</button><button type="button" data-l3="view" data-view="top">Bovenkant</button><button type="button" data-l3="overview">Alle stellingen</button></div><div class="l3-canvas"></div><div class="l3-stage-caption"><span>Sleep: draaien · Scroll: zoomen · Rechtermuisknop: verschuiven</span><button type="button" data-l3="fit">Alles in beeld</button></div><div class="l3-scale">Vloerraster 50 × 50 cm</div></div><aside class="l3-inspector" aria-label="Afmetingen en producten"></aside></div><div class="l3-flat" hidden></div></section>`;
    ...
    function normalize(data){
      ...
      data.racks.forEach((r,i)=>{r.rows=Number(r.rows);r.rowColumns=Array.from({length:r.rows},(_,y)=>Number(r.rowColumns?.[y]??r.columns));data.geometry.racks[r.id]??={width:200,height:200,depth:60,x:Math.min(5000,i*260),z:0,angle:0};});
    ...
    function inspector(){
      ...
      ${p?`<div class="l3-fields">${field('Breedte','width',pg.width,.1,500,'product')}${field('Hoogte','height',pg.height,.1,500,'product')}${field('Diepte','depth',pg.depth,.1,500,'product')}</div><p class="l3-muted">Maten van het product of de verpakking die je in dit vak neerzet. Eén blok per product, onafhankelijk van de voorraad.</p>`:`
      ...
      <details class="l3-section"><summary>Positie in het magazijn</summary><div class="l3-fields">${field('Links / rechts','x',g.x,-5000,5000)}${field('Voor / achter','z',g.z,-5000,5000)}${field('Draaien','angle',g.angle,0,359,'rack','1')}</div></details>
    ...
    root.addEventListener('click',async e=>{
      ...
      if(action==='overview'){state.overview=!state.overview;b.textContent=state.overview?'Geselecteerde stelling':'Alle stellingen';draw();view?.fit();return;}
      ...
      if(action==='add'){
        ...
        const id=crypto.randomUUID();state.data.racks.push({id,name:`Stelling ${n}`,rows:3,columns:3,rowColumns:[3,3,3]});state.data.geometry.racks[id]={width:200,height:200,depth:60,x:Math.min(5000,(state.data.racks.length-1)*260),z:0,angle:0};state.rackId=id;state.x=null;state.y=null;state.productId=null;dirty();render();view?.fit();return;
      }
      if(action==='remove'){
