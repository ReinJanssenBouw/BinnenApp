...
export function createLocationView(host, onSelect) {
  ...
  const grid=new T.GridHelper(100,200,0xa8b8cd,0xcbd6e4);grid.position.y=-.02;scene.add(grid);
  ...
  function text(parent,label,x,y,z,width,height,color='#173252',bg='#ffffff'){
    ...
    const m=new T.Mesh(new T.PlaneGeometry(width,height),new T.MeshBasicMaterial({map:texture,side:T.DoubleSide}));m.position.set(x,y,z);parent.add(m);
  }
  function update(data,selection,all=false){
    ...
    for(const r of data.racks){
      ...
      for(const x of [-w/2,w/2])for(const z of [-d/2,d/2])box(group,.035,h,.035,x,h/2,z,0x315887,{rackId:r.id});
      for(let y=0;y<=r.rows;y++){
        box(group,w,.025,d,0,y*cellHeight+.0125,0,0xb8c4d1,{rackId:r.id});
        box(group,w,.04,.025,0,y*cellHeight+.02,d/2,0x315887,{rackId:r.id});
      }
  ...
  function fit(mode='perspective'){
    const bounds=new T.Box3().setFromObject(content);if(bounds.isEmpty())bounds.set(new T.Vector3(-1,0,-1),new T.Vector3(1,2,1));
    ...
    const direction=mode==='front'?new T.Vector3(0,.08,1):mode==='top'?new T.Vector3(0,1,.001):new T.Vector3(1,.65,1.4);
  ...
  resize();
  return {update,fit,resize,dispose(){disposed=true;observer.disconnect();controls.dispose();clear();grid.geometry.dispose();grid.material.dispose();renderer.dispose();renderer.domElement.remove();}};
}
